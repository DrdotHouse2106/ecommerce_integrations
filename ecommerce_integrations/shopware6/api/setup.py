"""Operator-facing setup endpoints for the Shopware Setting form.

Powers the health banner, safety mode button, webhook widget and the 5-step
setup wizard rendered in the Shopware Setting form. All endpoints require a
System Manager (see :func:`require_shopware_admin`).

These endpoints intentionally surface existing capabilities — they do not
implement new sync behaviour. They aggregate state into plain-language
summaries non-technical operators can act on.
"""
...
def _last_test_at(setting) -> str | None:
    # Try a few well-known field names; fall back to the most recent successful
    # "test_connection" log entry if no dedicated field is stored on the setting.
...
def _is_misconfigured(setting) -> bool:
    """Return True for misconfigurations that silently break inbound webhooks."""
...
def get_health() -> dict[str, Any]:
    """Return a plain-language health summary for the Shopware Setting form."""
    require_shopware_admin()
    setting = _get_setting()
    enabled = bool(setting.enable_shopware)

    if not enabled:
        return {
            "state": "grey",
            "message": _("Integration deaktiviert"),
            "detail": _("Schalte 'Enable Shopware' ein, um zu starten."),
    ...
    # Critical: silent webhook rejection or a recent failed test_connection log.
...
def apply_safety_mode() -> dict[str, Any]:
    """Disable outbound pushes and pause Catalog Mirror runs for Shopware.

    Leaves the integration enabled and inbound webhooks active — the goal is
    to stop ERPNext from writing to Shopware while we investigate.
    """
    require_shopware_admin()
    setting = _get_setting()
    changes: list[str] = []
    ...
    if setting.update_shopware_item_on_update:
        setting.update_shopware_item_on_update = 0
        changes.append("update_shopware_item_on_update")
    ...
    if frappe.db.exists("DocType", "Ecommerce Catalog Mirror"):
        mirror_names = frappe.get_all(
            ...
            filters={"backend": "Shopware", "is_active": 1},
            pluck="name",
        )
...
def apply_setup_wizard(payload: str) -> dict[str, Any]:
    """Finalise the 5-step setup wizard.

    ``payload`` is a JSON string carrying connection credentials, the chosen
    sync mode and the webhook secret. Writes them to the Setting doc, ensures
    a webhook secret exists, and enables the integration.
    """
    require_shopware_admin()
    ...
    # --- Step 1: connection
    if data.get("shop_url"):
        setting.shop_url = data["shop_url"]
    if data.get("grant_type"):
        setting.grant_type = data["grant_type"]
    if data.get("client_id"):
        setting.client_id = data["client_id"]
    if data.get("client_secret"):
        setting.client_secret = data["client_secret"]
    if data.get("api_username"):
        setting.api_username = data["api_username"]
    if data.get("api_password"):
        setting.api_password = data["api_password"]

    # --- Step 3: sync mode -> internal toggles
    mode = (data.get("sync_mode") or "receive").lower()
    if mode == "full":
        setting.upload_erpnext_items = 1
        setting.update_shopware_item_on_update = 1
    else:
        setting.upload_erpnext_items = 0
        setting.update_shopware_item_on_update = 0
    ...
    # --- Step 4: webhook secret (auto-generate if absent)
...
def fire_test_webhook() -> dict[str, Any]:
    """POST a synthetic event to our own webhook endpoint to verify reach-back.

    The signature is computed from the configured secret so the round-trip
    exercises the signature validation path.
    """
    require_shopware_admin()
    setting = _get_setting()
    ...
    if not setting.enable_shopware:
        return {
            "success": False,
            "message": _("Shopware-Integration ist nicht aktiviert."),
        }
    ...
    if secret:
        ...
        headers["X-Shopware-Signature"] = signature
