    ...
    def validate(self):
        if self.enabled:
            ...
            if not self.pinecone_api_key:
                frappe.throw(_("Pinecone API Key is required"))
            if not self.pinecone_index_name:
                frappe.throw(_("Pinecone Index Name is required"))

            # Embedding provider validation
            if self.embedding_provider == "Google":
                if not self.google_api_key:
                    frappe.throw(_("Google API Key is required for Google embeddings"))
            elif self.embedding_provider == "OpenAI":
                if not self.openai_api_key:
                    frappe.throw(_("OpenAI API Key is required for OpenAI embeddings"))
