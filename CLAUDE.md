...
- **Stores ERP→backend external IDs in `tabEcommerce Item`** (canonical, `integration='shopware6'|'medusa'` — Module Def names, not the user-facing backend label) **AND on `Item Group.shopware_category_id` / `medusa_category_id`** (for Catalog Mirror's IG→category tree mapping). The two storage locations are not duplicates: `tabEcommerce Item` maps Items, the IG custom fields map category nodes. The Product Sync orchestrator routes the backend label (`"Shopware"`, `"Medusa"`) through `BACKEND_TO_INTEGRATION_KEY` in `product_sync/constants.py` to get the right Module Def name — never `(backend or "").lower()`, which silently writes `integration='shopware'` rows that fail Link validation on `Module Def`.
- **Item doc-events only queue when a Product Sync scope matches.** Since the `Ecommerce Product Sync` doctype landed, `Item.save()` still fans out to `shopware6.bulk_sync.queue_item_for_sync` and `medusa.bulk_sync.queue_item_for_sync`, but those handlers early-return when the Item is not in any active Product Sync's scope (checked via `product_sync.resolver.find_active_syncs_covering_item`). This is deliberate — the Product Sync doctype is now the explicit opt-in for "which Items get pushed". The check fails open on lookup errors so transient DB hiccups don't silently drop pushes.
